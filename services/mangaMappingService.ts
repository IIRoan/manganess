...
export async function getMangaMappings(): Promise<Record<string, MangaMapping>> {
    try {
        ...
        return mappings.reduce((acc, [key, value]) => {
            if (value) {
...
export async function getMangaMapping(internalId: string): Promise<MangaMapping | null> {
    try {
        const mapping = await AsyncStorage.getItem(`${MAPPING_PREFIX}${internalId}`);
